...
// get_completion 函数封装  通用的LLM 聊天完成接口函数， 复用
// async 是es7 提供的
/**
 * 异步函数，用于获取OpenAI的聊天完成结果。
 * @param {string} prompt - 用户输入的提示信息。
 * @param {string} [model='gpt-3.5-turbo'] - 使用的模型，默认为'gpt-3.5-turbo'。
 * @returns {Promise<string>} - 返回聊天完成结果的内容。
 */
const get_completion = async (prompt, model = 'gpt-3.5-turbo') => {
    ...
    // 调用OpenAI的聊天完成API，传入模型、消息和温度参数
    const response = await client.chat.completions.create({
        model: model,
        messages: messages,
        temperature: 0 // 温度参数设置为0，以获得最确定的结果
    });
...
const main = async () => {
    ...
    // nlp  情感分析
    const sentiment = 'negative'; // negative 消极
    /*
    const review =
     `
